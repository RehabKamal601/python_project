...
def apply_for_loan(user_id):
    ...
    cur.execute("INSERT INTO loans (user_id, amount) VALUES (%s, %s)", (user_id, amount))
    conn.commit()
    print("Loan application submitted.")

def make_payment(user_id):
    cur.execute("SELECT l.id, l.amount - COALESCE(SUM(p.amount), 0) AS balance FROM loans l LEFT JOIN payments p ON l.id = p.loan_id WHERE l.user_id = %s GROUP BY l.id", (user_id,))
    loans = cur.fetchall()
    if not loans:
...
def check_balance(user_id):
    cur.execute("""
        ...
        GROUP BY l.id
    """, (user_id,))
    for loan_id, amount, paid in cur.fetchall():
